...
bool next_fit(int *mem, processo *proc, int *lastplaced){
	int volta=-1;
	bool var=false,instr=false;
	for(int i=*lastplaced;i<MEMSIZE;i++){
		if(var && instr){
			return true;
		}
		if(i==*lastplaced){
		...
		if(!instr){
			if(mem[i]==-1){
				for(int j=0;j<proc->num_inst*2;j++){
					if(mem[i+j]!=-1){
						break;
					} else if (j==proc->num_inst*2-1) {
						proc->local_i=i;
						instr=true;
						if(i+j+1>=MEMSIZE){
							i=0;
...
/*void run_processes(processo **procs,int num_proc){
...
int main(int argc, char *argv[]){
	int num_proc=0, num_inst=0,lastplaced=0;
	...
	if(next_fit(MEM, procs[0], &lastplaced)){
		put_mem(MEM, procs[0]);
	}
	...
	if(next_fit(MEM, procs[1], &lastplaced)){
		put_mem(MEM, procs[1]);
	}
	...
	if(next_fit(MEM, procs[2], &lastplaced)){
		put_mem(MEM, procs[2]);
	}
	...
	if(next_fit(MEM, procs[3], &lastplaced)){
		put_mem(MEM, procs[3]);
	}
	...
	if(next_fit(MEM, procs[4], &lastplaced)){
		put_mem(MEM, procs[4]);
	}
	...
	if(next_fit(MEM, procs[5], &lastplaced)){
		put_mem(MEM, procs[5]);
	}
	...
	if(next_fit(MEM, procs[6], &lastplaced)){
		put_mem(MEM, procs[6]);
	}
	...
	if(next_fit(MEM, procs[7], &lastplaced)){
		put_mem(MEM, procs[7]);
	}
